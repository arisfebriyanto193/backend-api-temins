...
include '../../db/sql.php'; // Sesuaikan path database anda
include '../../auth/jwt.php'; // Sesuaikan path auth anda
...
function getSensorConfig($label) {
    ...
    if (strpos($l, 'lembab') !== false) return ['icon'=>'fa-droplet', 'color'=>'#10b981']; 
    if (strpos($l, 'tekanan') !== false) return ['icon'=>'fa-gauge', 'color'=>'#64748b']; 
    if (strpos($l, 'solar') !== false) return ['icon'=>'fa-sun', 'color'=>'#eab308']; 
    if (strpos($l, 'volt') !== false) return ['icon'=>'fa-car-battery', 'color'=>'#22c55e']; 
...
// $years = [];
// for ($y = $maxYear; $y >= $minYear; $y--) { $years[] = $y; }
