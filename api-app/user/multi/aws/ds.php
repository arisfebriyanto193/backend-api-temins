...
$requested_device_id = isset($_GET['device_id']) ? $_GET['device_id'] : '';

if (empty($requested_device_id)) {
    echo json_encode(["status" => false, "message" => "Device ID (unique_id) required"]);
    exit;
}

// 2. GET DEVICE INFO & VALIDASI OWNERSHIP
// Pastikan device dengan unique_id tersebut milik user ini
$q_device = mysqli_query($conn, "SELECT * FROM user_devices WHERE user_id = '$user_id' AND device_unique_id = '$requested_device_id' LIMIT 1");
$device = mysqli_fetch_assoc($q_device);

if (!$device) {
    // Cek apakah user adalah admin (opsional, jika admin boleh akses semua)
    // Untuk saat ini strict ke owner
    echo json_encode(["status" => false, "message" => "Device not found or access denied"]);
    exit;
}
