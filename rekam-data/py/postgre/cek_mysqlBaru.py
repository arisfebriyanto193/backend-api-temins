...
def get_all_sensor_heights_from_mysql():
    ...
    try:
        ...
        for row in results:
            device_id = row[0]
            tinggi_sensor = float(row[1]) if row[1] is not None else None
...
def auto_refresh_sensor_cache():
    ...
    while True:
        ...
        try:
            ...
            if changes_detected:
                print(f"\n{'='*70}")
                print(f"🔄 AUTO-REFRESH REPORT #{iteration} - {time.strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"{'='*70}")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🚀 TEMINS IoT Logger with Auto-Refresh MySQL Cache")
    print("="*60 + "\n")
