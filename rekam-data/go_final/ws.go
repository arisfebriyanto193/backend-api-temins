...
// LogMessage adalah struktur pesan yang dikirim ke WebSocket client.
...
// broadcastLog mengirim LogMessage ke semua WebSocket client yang terkoneksi.
// Client yang error (putus) akan langsung dihapus dari daftar.
...
// LogInfo mengirim log level "info" ke semua WebSocket client.
func LogInfo(msgType, message string, data map[string]interface{}) {
	broadcastLog(LogMessage{Type: msgType, Level: "info", Message: message, Data: data})
}

// LogSuccess mengirim log level "success" ke semua WebSocket client.
func LogSuccess(msgType, message string, data map[string]interface{}) {
	broadcastLog(LogMessage{Type: msgType, Level: "success", Message: message, Data: data})
}

// LogWarning mengirim log level "warning" ke semua WebSocket client.
func LogWarning(msgType, message string, data map[string]interface{}) {
	broadcastLog(LogMessage{Type: msgType, Level: "warning", Message: message, Data: data})
}

// LogError mengirim log level "error" ke semua WebSocket client.
func LogError(msgType, message string, data map[string]interface{}) {
	broadcastLog(LogMessage{Type: msgType, Level: "error", Message: message, Data: data})
}

// updateFlushStats memperbarui statistik flush (dipanggil setelah setiap flush berhasil).
...
// handleWebSocket meng-upgrade HTTP ke WebSocket, mendaftarkan client baru,
// mengirim stats awal, dan mempertahankan koneksi sampai client disconnect.
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	// Kirim statistik awal saat client connect
	flushStats.RLock()
	...
	// Loop baca untuk mendeteksi disconnect
	for {
...
// handleStats mengembalikan snapshot statistik sistem dalam format JSON.
// Endpoint: GET /stats
...
// StartLogServer memulai HTTP server untuk WebSocket log dashboard.
// Route:
//   - GET /         → HTML dashboard monitor
...
func StartLogServer(port int) {
	...
	// Sajikan HTML dashboard monitoring
...
<body>
    ...
    <script>
        ...
        setInterval(() => {
            fetch('/stats').then(r => r.json()).then(updateStats).catch(console.error);
        }, 10000);
